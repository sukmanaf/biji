...
export default function Page() {
  ...
  const handleEditRecord = async (data: FormData) => {
    try {
      const response = await fetch(`/api/tahun_ajaran/${idEdit}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleEdit = async (id: number) => {
    try {
      ...
      const record: FormData = await response.json();
  ...
  const handleView = async (id: number) => {
    try {
      setIdEdit(id)
      const response = await fetch(`/api/tahun_ajaran/view/${id}`);
      if (!response.ok) {
        throw new Error("Failed to fetch record");
      }
  ...
  return (
    <ContentLayout title="Tahun Ajaran">
      <Breadcrumb>
        <BreadcrumbList>
          <BreadcrumbItem>
